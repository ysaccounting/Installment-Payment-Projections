...
def build_filename(df, fmt):
    if fmt == 'old':
        ...
        date_range_str = str(df.iloc[2].dropna().tolist()[0]).strip()
    month_m   = re.search(r'(\w+)\s+(\d+)', date_range_str)
    year_m    = re.search(r'(\d{4})', date_range_str)
    month_str = month_m.group(1)
    start_day = int(month_m.group(2)) if year_m and month_m.group(2) != year_m.group(1) else 1
    year_str  = year_m.group(1) if year_m else None
    current_month = pd.to_datetime(f'{month_str} 1 {year_str}')
    next_month = current_month + pd.offsets.MonthBegin(1)
    target = next_month.replace(day=start_day)
    day = target.day
    suffix = {1:'st',2:'nd',3:'rd'}.get(day%10,'th') if day not in (11,12,13) else 'th'
    date_label = target.strftime(f'%B {day}{suffix}')
